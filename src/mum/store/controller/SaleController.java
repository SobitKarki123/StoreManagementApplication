...
import mum.store.bal.PurchaseManager;
import mum.store.bal.SaleManager;
...
public class SaleController implements Initializable {
	...
	public void initialize(URL url, ResourceBundle resources) {
		// products = FXCollections.observableArrayList(new Product("test",
		// "CC1", 90.0, Long.parseLong("1")),
		// new Product("tast", "CC2", 40.0, Long.parseLong("2")));
		ProductManager pm = new ProductManager();
		products = FXCollections.observableArrayList(pm.getAllProducts());
	...
	private void addSale() {
		...
		SaleManager sman = new SaleManager();

		String billNumber = saleBillNumberTextField.getText();
		LocalDate date = saleBillDate.getValue();
		Double totalAmount = Double.parseDouble(sale_totalAmount.getText());
		SaleMaster sd = new SaleMaster(billNumber, date.toString(), totalAmount);

		sman.insertSale(saleItems, sd);
